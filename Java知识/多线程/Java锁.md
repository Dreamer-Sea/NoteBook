...
## 锁优化
在较早的JDK版本种，synchronized并不是特别高效，但是随着JDK的更新迭代，synchronized的表现得到了改进，最新的synchronized做了以下的优化。
偏向锁、轻量级锁、自旋锁、适应性自旋锁、锁销除、锁粗化等。
## 锁升级
锁从偏向锁升级到轻量级锁的过程，是依据锁对象的对象头的**threadid字段**。当锁从无锁升级为偏向锁时，会将threadid设置为第一个获取到它的线程的id。如果下一次获取锁的线程的id与锁的threadid不一致，锁就会升级为轻量级锁。并且在执行**一定次数**后升级为重量级锁。
## 锁的三种使用方式
1. 修饰实例方法：给实例对象加锁。
2. 修饰静态方法：给整个类加锁。
3. 修饰代码块：给指定对象/类加锁。
