...
## Android的多进程？
可以在AndroidManifest.xml文件中给四大组件指定属性android:process开启多进程模式。

## 为什么需要进行IPC？多进程通信可能会出现什么问题？
1. **多进程造成的影响**可以总结为以下四个方面：
...
2. **需要进程间通信的必要性：**所有运行在不同进程的四大组件，只要它们之间需要通过内存共享数据，都会共享失败。这是由于android为每个应用分配了**独立的**虚拟机，不同的虚拟机在内存分配上有不同的地址空间，这会导致在不同的虚拟机中访问同一个类的对象会产生多份副本。

## 什么是序列化？Serializable接口和Parcelable接口的区别？为何推荐使用后者？
序列化表示将一个对象转换成**可存储或可传输**的状态。
后者能够将对象进行分解，且分解后的每一部分都是传递可支持的数据类型。

## Android中为何新增Binder来作为主要的IPC方式？
- 传输效率高，可操作性强：消息队列，Socket和管道传输时需要将数据拷贝两次，数据先从发送方的缓存区拷贝到内核开辟的缓存区中，再从内核缓存区拷贝到接收方的缓存区。而在Binder中，**接收方缓存区与内核缓存区是映射到用一块物理地址的**，这样就能节省一次数据拷贝。
- 实现C/S架构方便：Linux中的IPC方式除了Socket以外，其它的方式都不是基于C/S架构的，而Socket**主要用于网络间的通信**且**传输效率低**。Binder基于C/S架构，Server端与Client端相对独立，稳定性好。
- 安全性高：传统的Linux IPC的接收方无法获得对方进程可靠的UID/PID，从而无法鉴别对方身份；而Binder机制为每个进程分配了UID/PID且在Binder通信时会根据UID/PID进行有效性检测。

## 使用Binder进行数据传输的具体过程？
服务端中的Service给与其绑定的客户端提供Binder对象，客户端通过AIDL接口中asInterface()将这个Binder对象转换为代理Proxy，并通过它发起RPC请求。客户端发起请求时会挂起当前线程，并将参数写入data然后调用transact()，RPC请求会通过系统底层封装后由服务器端的onTransact()处理，并将结果写入reply，最后返回调用结果并唤醒客户端线程。
